#!/usr/bin/env python3
"""Build the two-page cover that leads the Frog's First Key Jumps printable.

    pip install reportlab pymupdf qrcode
    python3 tools/build-interval-jump-key-jumps-cover.py <cover.pdf>
    python3 tools/build-interval-jump-key-jumps-cover.py --art <fan.png>   # transparent

The sibling of `build-interval-jump-frogs-cover.py`: same landscape Bees Keys
cover grid (eyebrow, hairline rule, light title, a fan of whole sheets,
centred caption, footer bar), same page two, only the sheets and the words
change. Frog's First Jumps put repeat, step, and skip on the staff; this set
puts them on the piano keys (eight little F-G-A-B keyboards floating on the
river, Frog on the key it starts from, a ladybug on the key it wants).

The fan shows two of the four sheets whole, per Josh's standing preference:
the full-colour circle sheet in front with the printer-friendly write-in
behind it - one of each version and one of each style, so the caption's
"full-colour and ink-friendly" claim is visible rather than asserted.

Page two's claims come from the app (`model/Game.swift`,
`view/settings/Settings.swift`): repeat, step, and skip are free forever;
leap, the ordinal 4th-9th intervals, custom sessions, and the timers are the
Pro one-time unlock. The game itself is staff-only (no keyboard mode), so the
copy says the worksheets practise the same three jumps, not the same picture.
...
OUT = sys.argv[1] if len(sys.argv) > 1 and not sys.argv[1].startswith("-") \
    else "/tmp/interval-jump-frogs-first-key-jumps-cover.pdf"
...
def page_one(c, circle, write_in_ink):
    ...
    c.setFillColor(INK)
    c.setFont("Nunito", 28)
    c.drawString(42, H - 92, "Frog’s First Key Jumps")
    c.setFillColor(AZUL)
    c.setFont("Nunito", 9.5)
    c.drawString(43, H - 111, "REPEAT  •  STEP  •  SKIP  •  ON THE KEYS")
    ...
    c.setFillColor(INK)
    c.setFont("Nunito", 12)
    c.drawCentredString(W / 2, 100, "4 keyboard worksheets for finding repeat, step, and skip on the keys")
...
def page_two(c, qr_ios, qr_play):
    ...
    para(c,
         "Interval Jump takes the same three jumps from the keys to the staff: "
         "repeat, step, and skip, log to log across the river.",
         66, 468, 440, 8.8, 12)
...
def main():
    circle = sheet_page("interval-jump-frogs-first-key-jumps.pdf")
    write_in_ink = sheet_page("interval-jump-frogs-first-key-jumps-write-in-no-background.pdf")
    ...
    c.setTitle("Frog's First Key Jumps - Interval Jump")
